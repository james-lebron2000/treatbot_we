...
const buildRedisClient = () => {
  ...
  // 必须挂 'error' 监听：否则 Redis 不可达时 ioredis 抛 unhandled 'error' 事件 → 直接 crash 进程
  // （限流 / 幂等 / 刷新令牌共用此客户端）。挂上后退化为"命令失败"而非"进程退出"。
  client.on('error', (err) => {
...
const createRateLimiter = (options = {}) => {
  ...
  // 多实例 / 重启后仍生效：优先用 Redis 共享存储（生产已依赖 Redis 做幂等/刷新令牌），
  // 未配置 REDIS_HOST（本地/测试）时回退进程内存 MemoryStore。
  let store;
  ...
  return rateLimit({
    ...
    legacyHeaders: false,
    keyGenerator: (req) => {
      // 优先使用用户 ID，其次是 IP；前缀由 store 统一添加，避免双重前缀
