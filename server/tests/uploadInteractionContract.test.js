...
describe('startMatching 保存条件 — fileId+(missing>0||gapDirty) 才 PATCH，并清匹配缓存', () => {
  ...
  test('selectGapOption 在用户点选项时也翻 dirty', () => {
    // 下拉/胶囊选项是另一个字段编辑入口；只覆盖 input 会漏掉"点选项补齐"这条真实路径。
    const selectGapOptionBlock = src.match(/selectGapOption\s*\(\s*e\s*\)\s*\{[\s\S]*?\n\s\s\}/);
    expect(selectGapOptionBlock).toBeTruthy();
    expect(selectGapOptionBlock[0]).toMatch(/setData\(\s*\{\s*gapDirty:\s*true\s*\}/);
  });
  ...
  test('startMatching 用 (missing>0 || gapDirty) && fileId 决定是否 PATCH', () => {
    ...
    expect(block).toMatch(/currentRecordId/);
    // 反向 guard：旧条件不能再独占 —— 必须有 || gapDirty 或等价的 OR 分支
    expect(block).toMatch(/missingFields\.length\s*>\s*0\s*\|\|\s*gapDirty/);
    expect(block).toMatch(/const\s+shouldPersist\s*=\s*!!currentRecordId\s*&&/);
  });

  test('保存 PATCH 使用 currentRecordId（recordId 优先，fileId 兜底）', () => {
    const startMatchingBlock = src.match(/async\s+startMatching\s*\(\s*\)\s*\{[\s\S]*?\n\s\s\},/);
    expect(startMatchingBlock).toBeTruthy();
    expect(startMatchingBlock[0]).toMatch(/const\s+currentRecordId\s*=\s*recordId\s*\|\|\s*fileId\s*\|\|\s*''/);
    expect(startMatchingBlock[0]).toMatch(/api\.enrichMedicalRecord\(\s*currentRecordId\s*,\s*parsedData\s*\)/);
  });
