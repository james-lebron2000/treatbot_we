...
// openid 单一来源：手机号通道（短信 / 账号密码）统一用 treatbot_${phone}，与 treatbotLogin 对齐，
// 保证同一手机号无论走短信、密码还是后续绑定，都落在同一个 User 行。
const treatbotOpenidForPhone = (phone) => `treatbot_${phone}`;
 ...
 * - openid = treatbot_${phone}，与短信/微信登录共用同一 User 行：
...
const register = async (req, res, next) => {
  try {
    ...
    const openid = treatbotOpenidForPhone(normalizedPhone);
...
const passwordLogin = async (req, res, next) => {
  try {
    ...
    const user = await User.findOne({ where: { openid: treatbotOpenidForPhone(normalizedPhone) } });
